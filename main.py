...
def music():
    ...
    if 'despacito' in musicName:
        os.system(' /Users/ramshinde/Downloads/Despacito\(PaglaSongs\).mp3 ')
    else:
...
def openApps(query):
    ...
    if 'whatsapp' in query:
        query = query.replace("open whatsapp and send message to"," ")
        query = query.replace("and send", "")
...
def TaskExe():
    ...
    while True:
         ...
         #     Clock_time(query)
             ...
             keyboard.press_and_release('command + r')
         ...
         elif 'increase audio' in query:
             keyboard.press_and_release('command + f12')
